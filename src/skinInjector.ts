...
async function get_repo_skin_url_if_exist(champNum: string, skin_number: string) {
    const owner = 'darkseal-org';
    const repo = 'lol-skins';
    const branch = 'main';
    let champ_skin_name = await getChampSkinNames(champNum, skin_number)
    let skin_name_path_in_repo = `${champ_skin_name[0]}/${champ_skin_name[1].replace(/[/]/g, " ").replace(":", "")}`;
...
async function download_skin(url:string,outputPath:string):Promise<boolean>{
    try {
